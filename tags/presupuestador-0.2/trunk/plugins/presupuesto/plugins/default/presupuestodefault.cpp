...
void PresupuestoDefault::regenerar( const QTextDocument *docCont )
{
 if( archivo == 0 )
 {
  ...
  QString *html = new QString( archivo->readAll() );
  archivo->close();
  QTextCursor *cursor = new QTextCursor( _doc );
 ...
 _doc->setHtml( docCont->toHtml() );
}


/*!
    \fn PresupuestoDefault::inicializar()
 */
void PresupuestoDefault::inicializar()
{
...
/*!
    \fn PresupuestoDefault::limpieza
 */
void PresupuestoDefault::limpieza()
{
 delete _doc;
 if( archivo != 0 )
 {
