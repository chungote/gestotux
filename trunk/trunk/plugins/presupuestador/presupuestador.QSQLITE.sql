...
CREATE TABLE IF NOT EXISTS `presupuesto` ( `id_presupuesto` INTEGER PRIMARY KEY AUTOINCREMENT, `fecha` TEXT NOT NULL, `kilometraje` INTEGER NOT NULL, `id_auto` INTEGER REFERENCES `autos`(`id_auto`), `total` NUMERIC NOT NULL,`titulo` text, `memo` text, `contenido` blob, `creado` text NOT NULL, `modificado` TEXT , `imprimir` INTEGER, `email` INTEGER );
