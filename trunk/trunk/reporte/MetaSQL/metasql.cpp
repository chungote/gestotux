...
static const QVariant _trueVariant = QVariant(TRUE, 0);
static const QVariant _falseVariant = QVariant(FALSE, 0);
...
class MetaSQLFunction : public MetaSQLOutput {
    ...
    protected:
        Function identifyFunction(const QString & func) {
            QString f = func.stripWhiteSpace().lower();
            if(f == "value")
...
class MetaSQLBlock : public MetaSQLOutput {
            ...
            if(_block != BlockUnknown) {
                ...
                switch(_block) {
                    ...
                    case BlockElseIf:
                        ...
                        wip = _options.stripWhiteSpace();
                        if(wip.left(4).lower() == "not ") {
                            _if_not = true;
    ...
    protected:
        Block identifyBlock(const QString & block) {
            QString b = block.stripWhiteSpace().lower();
            if(b == "generic")
                return BlockGeneric;
            else if(b == "if")
                return BlockIf;
            ...
            qDebug("Unable to identify block '%s'!", (const char*)b);
