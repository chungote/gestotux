...
<h2><?php echo sprintf(__("Ver %s", true), $singularHumanName);?></h2>
	<dl>
<?php
$i = 0;
...
<div class="actions">
	<ul>
<?php
	echo "\t\t<li>" .$html->link(sprintf(__('Editar %s', true), $singularHumanName),   array('action'=>'edit', ${$singularVar}[$modelClass][$primaryKey])). " </li>\n";
	echo "\t\t<li>" .$html->link(sprintf(__('Eliminar %s', true), $singularHumanName), array('action'=>'delete', ${$singularVar}[$modelClass][$primaryKey]), null, __('Are you sure you want to delete', true).' #' . ${$singularVar}[$modelClass][$primaryKey] . '?'). " </li>\n";
	echo "\t\t<li>" .$html->link(sprintf(__('Listar %s', true), $pluralHumanName), array('action'=>'index')). " </li>\n";
	echo "\t\t<li>" .$html->link(sprintf(__('Nuevo/a %s', true), $singularHumanName), array('action'=>'add')). " </li>\n";
...
?>
<div class="related">
	<h3><?php echo sprintf(__("Relacionado %s", true), Inflector::humanize($_details['controller']));?></h3>
<?php if (!empty(${$singularVar}[$_alias])):?>
...
<?php
		...
		echo "\t\t<tr{$class}>\n";
			...
			echo "\t\t\t\t" . $html->link(__('Ver', true), array('controller'=> $_details['controller'], 'action'=>'view', ${$otherSingularVar}[$_details['primaryKey']])). "\n";
			echo "\t\t\t\t" . $html->link(__('Editar', true), array('controller'=> $_details['controller'], 'action'=>'edit', ${$otherSingularVar}[$_details['primaryKey']])). "\n";
			echo "\t\t\t\t" . $html->link(__('Eliminar', true), array('controller'=> $_details['controller'], 'action'=>'delete', ${$otherSingularVar}[$_details['primaryKey']]), null, __('Are you sure you want to delete', true).' #' . ${$otherSingularVar}[$_details['primaryKey']] . '?'). "\n";
			echo "\t\t\t</td>\n";
		echo "\t\t</tr>\n";
		endforeach;
?>
...
<?php endif; ?>
	<div class="actions">
		<ul>
			<li><?php echo $html->link(sprintf(__("Nuevo/a %s", true), Inflector::humanize(Inflector::underscore($_alias))), array('controller'=> $_details['controller'], 'action'=>'add'));?> </li>
		</ul>
	</div>
</div>
<?php endforeach;?>