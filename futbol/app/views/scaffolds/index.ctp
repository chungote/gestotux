...
echo $paginator->counter(array(
	'format' => 'Pagina %page% de %pages%, mostrando %current% registros de %count% en total, empezando en el registro %start%, terminando en %end%'
));
?></p>
...
echo "\n";
	echo "\t<tr{$class}>\n";
		...
		echo "\t\t\t" . $html->link(__('Ver', true), array('action'=>'view', ${$singularVar}[$modelClass][$primaryKey])) . "\n";
	 	...
	 	echo "\t\t\t" . $html->link(__('Eliminar', true), array('action'=>'delete', ${$singularVar}[$modelClass][$primaryKey]), null, __('Esta seguro que desea eliminar', true).' #' . ${$singularVar}[$modelClass][$primaryKey]) . "\n";
		echo "\t\t</td>\n";
	echo "\t</tr>\n";
...
<div class="actions">
	<ul>
		<li><?php echo $html->link('Nuevo/a '.$singularHumanName, array('action'=>'add')); ?></li>
<?php
		$done = array();
