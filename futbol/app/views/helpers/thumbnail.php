...
class ThumbnailHelper extends Helper {
	...
	private function init($options = array(), $tag_options = array())	{
		...
		$this->options['save_path'] = WWW_ROOT . 'img'. DS . 'galeria' . DS . 'thumbs';
		$this->set_file_extension();
		$this->set_cache_filename();
		$this->error = '';
		$this->options['display_path'] =  'galeria/thumbs';
	...
	private function show_image_tag()	{
		if($this->error != '')	{
			...
			$src = $this->url( '/' ) . IMAGES_URL . $this->options['display_path'] . '/' . substr($this->cache_filename, strrpos($this->cache_filename, DS) + 1, strlen($this->cache_filename));
		}
		$img_tag = '<img src="' . $src . '"';
